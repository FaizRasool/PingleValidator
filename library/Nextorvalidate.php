...
class validation{
    ...
    */
        ...
        foreach( new ArrayIterator($this->validation_rules) as $var=>$opt)
        {
            ...
            switch($opt['type'])
            {
                ...
                case 'bool':
                    ...
                    case 'regex':
                    $this->validateregex($var, $opt['required'],$opt['regex']);
                    if(!array_key_exists($var, $this->errors))
                    {
                        $this->sanitizeString($var);
                    }
     ...
     *
     */
    public function addRule($varname, $type, $required=false, $min=0, $max=0, $trim=false,$regex="")
    {
    ...
    private function validateregex($var, $required=false,$regex)
    {
        if($required==false && strlen($this->source[$var]) == "")
        {
            return true;
        }
    ...
    $this->sanitized[$var] = filter_var($this->source[$var], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);    
     }
    /**
     *
     * @sanitize a string
